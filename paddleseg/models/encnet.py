...
class ENCNet(nn.Layer):
    ...
    def forward(self, inputs):
        ...
        out = F.interpolate(out, scale_factor=self.up_scale, mode='bilinear', align_corners=False)
        output = [out]
        if self.training:
            fcn_out = self.fcn_head(fcn_feat)
            fcn_out = F.interpolate(fcn_out, scale_factor=self.up_scale, mode='bilinear', align_corners=False)
